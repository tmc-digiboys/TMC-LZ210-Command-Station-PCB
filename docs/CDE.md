...
### Normal operation
If no short circuit is reported, the circuit operates as follows. When the **C** signal is high, the 47 nF capacitor (C701) is charged via the 4.7 kΩ resistor (R703). The time required to charge the capacitor to its maximum voltage depends on the voltage and the precise shape of the DCC signal, but is in the order of a few milliseconds. The 1N4148 diode (D701) prevents the capacitor from discharging when the **C** signal is low.

To obtain a suitable input voltage for a 3V3 microcontroller, the voltage across the capacitor is reduced to approximately 28% of the original voltage using a voltage divider comprising 120 kΩ (R704) and 47 kΩ (R705) resistors. At a maximum **C** voltage of **12 V**, the node charges to: *V<sub>node</sub> = (12 − 0,7) × 47 / (120 + 47 + 4,7) ≈ 3,1 V*. The 3.3 V Zener diode (D702) protects the microcontroller input against potential excessive voltage spikes. The 100 pF capacitor (C702), together with the resistor divider, forms a low-pass filter with a cut-off frequency of approximately 50 kHz. This filter suppresses any high-frequency interference at the microcontroller’s input.

If a 5 V microcontroller is used instead of a 3.3 V microcontroller, the 47 kΩ resistor must be replaced with a 68 kΩ resistor and the 3.3 V Zener diode (D702) with a 5 V Zener diode.

### Short circuit

During a short circuit or emergency stop, the 47 nF capacitor may discharge via the **E** terminal, the optocoupler and the diode in the booster, to **D**. This discharge only occurs when the **D** signal is low, so for a maximum of 58 (DCC 1) or 100 (DCC 0) µs each time.

The voltage at **E** drops to the sum of the diode’s forward voltage and the optocoupler’s saturation voltage (*V*<sub>CE(sat)</sub>). This is typically around 0.7 to 1.1 V: approximately 0.6 V for the diode and 0.1 to 0.5 V for the optocoupler, depending on the degree to which the phototransistor is saturated. As the voltage divider reduces the voltage across the capacitor to approximately 28% of its original value, the voltage at the microcontroller input during a short circuit is typically only 0.2 to 0.4 V.
