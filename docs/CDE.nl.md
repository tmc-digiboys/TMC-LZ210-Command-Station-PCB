...
### Normaal bedrijf
Als er geen kortsluiting wordt gemeld, werkt de schakeling als volgt. Wanneer het **C**-signaal hoog is, wordt de 47 nF-condensator (C701) via de 4,7 kΩ-weerstand (R703) opgeladen. De benodigde tijd om de condensator tot vrijwel zijn maximale spanning op te laden, hangt af van de spanning en de precieze vorm van het DCC-signaal, maar ligt in de orde van enkele milliseconden. De 1N4148-diode (D701) voorkomt dat de condensator zich ontlaadt wanneer het **C**-signaal laag is.

Om een geschikte spanning voor een 3V3-microcontrolleringang te verkrijgen, wordt de spanning over de condensator met een spanningsdeler van 120 kΩ (R704) en 47 kΩ (R705) verlaagd tot ongeveer 28% van de oorspronkelijke spanning. Bij een maximale **C** spanning van **12 V** laadt het knooppunt op tot: *V<sub>node</sub> = (12 − 0,7) × 47 / (120 + 47 + 4,7) ≈ 3,1 V*. De 3,3 V-zenerdiode (D702) beschermt de microcontrolleringang tegen te hoge spanningen. De 100 pF-condensator (C702) vormt samen met de weerstands­deler een laagdoorlaatfilter met een afsnijfrequentie van ongeveer 50 kHz. Dit filter onderdrukt eventuele hoogfrequente storingen op de ingang van de microcontroller.

Mocht in plaats van een 3V3-microcontroller een 5V-microcontroller worden gebruikt, dan moet de weerstand van 47 kΩ worden vervangen door een weerstand van 68 kΩ en de 3,3 V-zenerdiode (D702) door een 5 V-zenerdiode.

### Kortsluiting

Tijdens een kortsluiting of noodstop kan de 47 nF-condensator via de **E**-aansluiting, de optocoupler en de diode in de booster naar **D** ontladen. Dit ontladen kan alleen plaatsvinden wanneer het **D**-signaal laag is, dus steeds maximaal 58 (DCC-1) of 100 (DCC-0) µs.

De spanning op **E** daalt daarbij tot de som van de doorlaatspanning van de diode en de verzadigingsspanning (*V*<sub>CE(sat)</sub>) van de optocoupler. Deze bedraagt typisch ongeveer 0,7 tot 1,1 V: circa 0,6 V voor de diode en 0,1 tot 0,5 V voor de optocoupler, afhankelijk van de mate waarin de fototransistor verzadigd is. Omdat de spanningsdeler de spanning over de condensator verlaagt tot ongeveer 28% van de oorspronkelijke waarde, bedraagt de spanning op de microcontrolleringang bij een kortsluiting typisch slechts 0,2 tot 0,4 V.
